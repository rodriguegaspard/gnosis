...
impl Parser<Activity> for AgendaParser {
    fn parse(filepath: &str) -> Result<Vec<Activity>, io::Error> {
        ...
        let re = Regex::new(
            r"(?<title>.+);(?<start>[0-9]+);(?<end>[0-9]+);(?<description>.*);(?<priority>.+)",
        )
        .unwrap();
            ...
            match re.captures(line) {
                Some(caps) => {
                    if caps["start"] > caps["end"] {
                        ...
                        let start = Local
                            .timestamp_opt(caps["start"].parse::<i64>().unwrap(), 0)
                            .unwrap();
                        let end = Local
                            .timestamp_opt(caps["end"].parse::<i64>().unwrap(), 0)
                            .unwrap();
...
mod tests {
    ...
    fn different_week_offsets() {
        let now = Local::now();
        let dt1 =
            now - Duration::days(now.weekday().num_days_from_monday() as i64) - Duration::days(14);
    ...
    fn parse_agenda() {
        let activities: Vec<Activity> =
            AgendaParser::parse("tests/parser_test.txt").expect("Failed to open the test agenda");
        assert_eq!(2, activities.len());
        assert_eq!(
            (
                &"testevent1".to_string(),
                &Local
                    .timestamp_opt("1577808000".parse::<i64>().unwrap(), 0)
                    .unwrap(),
                &Local
                    .timestamp_opt("1578585600".parse::<i64>().unwrap(), 0)
                    .unwrap(),
